...
export async function exportConversationPdf(opts: ExportConversationPdfOpts): Promise<void> {
  ...
  const [clientLogoDataUrl, rendererModule, docModule, rasterizerModule, htmlVisualModule] =
    await Promise.all([
      ...
      import('@/components/content-lab/content-lab-conversation-pdf'),
      import('@/lib/content-lab/rasterize-mermaid'),
      import('@/lib/content-lab/rasterize-html-visual'),
    ]);
  const { pdf } = rendererModule;
  const { ContentLabConversationPdf } = docModule;
