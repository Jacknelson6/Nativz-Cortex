...
export function truncateStrategyLabContextPack(text: string, maxChars = MAX_PACK_CHARS): string {
  const trimmed = text.trim();
 ...
 */
export async function buildStrategyLabContextPack(
  ...
  try {
    ...
    return truncateStrategyLabContextPack(parts.join('\n'));
  } catch {
    return '';
  }
}
