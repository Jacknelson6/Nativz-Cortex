...
- **Push notify on remote sessions.** Whenever Jack is driving Cortex via remote control (Claude Code session he's not actively watching), call `PushNotification` whenever you reach a point that needs a decision, input, or "you're done, what's next?" Do not silently idle. Ping at the moment you'd otherwise sit waiting. Same rule for genuinely long-running blockers (browser auth, deploy approvals).
