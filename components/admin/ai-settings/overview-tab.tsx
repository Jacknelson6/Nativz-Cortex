...
async function loadStats(): Promise<OverviewStats> {
  ...
  const [agencyRes, skillsCountRes, apifyRes, apiUsageRes, scraperSettings] = await Promise.all([
    // Model + API key live on agency_settings (per /api/settings/ai-model + llm-credentials routes).
    admin.from('agency_settings').select('*').limit(1).maybeSingle(),
    admin.from('nerd_skills').select('id', { count: 'exact', head: true }).eq('is_active', true),
  ...
  // Pick the first non-empty model slug across the various routing slots.
  ...
  const apifySpend7dUsd = await safeSum(apifyRes.data);
  const modelSpend7dUsd = await safeSum(apiUsageRes.data);
  const apifyRuns7d = apifyRes.data?.length ?? 0;

  const { totalUsd: costPerSearchUsd } = estimateSearchCost(scraperSettings);
