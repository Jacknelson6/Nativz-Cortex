...
export function ContentLabArtifactsPanel({ clientId, clientName, agency }: ArtifactsPanelProps) {
  ...
  const handleExportPdf = useCallback(async () => {
    ...
    try {
      const [rendererModule, docModule] = await Promise.all([
        ...
        import('./content-lab-artifact-pdf'),
      ]);
      const { pdf } = rendererModule;
      const { ContentLabArtifactPdf } = docModule;

      const blob = await pdf(
        ContentLabArtifactPdf({
