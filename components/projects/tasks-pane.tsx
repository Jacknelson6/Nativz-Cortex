...
/**
 * Day-to-day individual to-do list. Lives inside /admin/projects under the
 * Tasks tab. Three sub-views (Today / Upcoming / All) driven by `?view=`.
 *
 * Originally lived at /admin/tasks; folded into the Project Management page
 * 2026-04-25 so PM and Tasks share a single nav home.
 */
export function TasksPane() {
  ...
  // URL params: keep `tab=tasks` so a refresh stays on this pane, then layer
  // view/assignee/client on top.
  ...
  function setView(v: ViewTab) {
    router.push(`/admin/projects?${buildParams({ view: v })}`, { scroll: false });
  }

  function handleAssigneeFilter(id: string) {
    setFilterAssigneeId(id);
    router.push(`/admin/projects?${buildParams({ assignee: id })}`, { scroll: false });
  }

  function handleClientFilter(id: string) {
    setFilterClientId(id);
    router.push(`/admin/projects?${buildParams({ client: id })}`, { scroll: false });
  }
  ...
  const subLabel = view === 'today' ? 'Today' : view === 'upcoming' ? 'Upcoming' : 'All tasks';
  ...
  return (
    <div>
      ...
      {/* Pane sub-header — sub-views (Today/Upcoming/All) + filters. The
          page-level title lives in the parent SectionHeader. */}
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 text-sm text-text-muted">
          <Check size={14} />
          <span>
            {activeCount} {activeCount === 1 ? 'task' : 'tasks'} · {subLabel}
          </span>
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="flex items-center gap-1">
            {(['today', 'upcoming', 'all'] as ViewTab[]).map((v) => (
              <button
                key={v}
                onClick={() => setView(v)}
