...
import { X, Wand2, Loader2, CheckCircle2, AlertCircle, Film } from 'lucide-react';
...
export function AutoScheduleDialog({
  ...
  // Auto-calculate posts per week from media count and date range
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-surface rounded-2xl border border-nativz-border shadow-2xl w-full max-w-lg flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-3 border-b border-nativz-border">
          ...
          {step !== 'processing' && (
            <button onClick={step === 'done' ? handleDone : onClose} className="cursor-pointer p-1 rounded-lg hover:bg-surface-hover text-text-muted">
              <X size={18} />
            </button>
          )}
        </div>
        ...
        {step === 'done' && (
          <>
            ...
            <div className="flex items-center justify-end px-5 py-3 border-t border-nativz-border">
              <GlassButton onClick={handleDone}>View calendar</GlassButton>
            </div>
          </>
        )}
      </div>
