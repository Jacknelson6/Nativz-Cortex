...
interface StrategyLabAttachClientDialogProps {
  open: boolean;
  onClose: () => void;
  searchId: string;
}
...
export function StrategyLabAttachClientDialog({
  open,
  onClose,
  searchId,
}: StrategyLabAttachClientDialogProps) {
  ...
  async function handlePick(client: ClientOption) {
    ...
    try {
      ...
      try {
        const key = strategyLabTopicSearchStorageKey(client.id);
