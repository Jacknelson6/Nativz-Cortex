...
import { AlertCircle, ArrowDownCircle, ArrowUpCircle, Loader2, PauseCircle, PlayCircle, RefreshCcw } from 'lucide-react';
...
export function CreditsAdminPanel({ clientId, balance, transactions }: Props) {
  ...
  const [allowance, setAllowance] = useState<string>(String(balance?.monthly_allowance ?? 0));
  const [rolloverPolicy, setRolloverPolicy] = useState<RolloverPolicy>(
    balance?.rollover_policy ?? 'none',
  );
  const [rolloverCap, setRolloverCap] = useState<string>(String(balance?.rollover_cap ?? 0));

  const isPausedIndefinite = balance?.auto_grant_enabled === false;
  const isPausedTimeBound =
    !!balance?.paused_until && new Date(balance.paused_until).getTime() > Date.now();
    ...
    <>
      ...
      {!hasBalance ? (
        <section className="rounded-xl border border-amber-300/30 bg-amber-300/5 p-5 text-sm text-amber-300">
          No credit balance row exists for this client. Run the credits backfill to create one.
        </section>
      ) : null}
      ...
      <section className="rounded-xl border border-nativz-border bg-surface p-5">
        ...
        <p className="mt-1 text-[12px] text-text-muted">
          Allowance is granted on the next reset after the period ends. Setting allowance does not
          immediately credit the account.
        </p>
        <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
          <label className="block">
            <span className="text-[11px] uppercase tracking-wider text-text-muted">Allowance</span>
            <input
              type="number"
              min={0}
              max={10000}
              ...
              disabled={!hasBalance || busy === 'allowance'}
            />
          </label>
          <label className="block">
          ...
          <label className={`block ${rolloverPolicy === 'cap' ? '' : 'opacity-50'}`}>
            ...
            <input
              ...
              className="mt-1 w-full rounded-lg border border-nativz-border bg-background px-3 py-2 text-sm text-text-primary focus:border-accent focus:outline-none"
              disabled={!hasBalance || busy === 'allowance' || rolloverPolicy !== 'cap'}
            />
          </label>
        </div>
        <div className="mt-4 flex justify-end">
          <Button
            ...
            disabled={!hasBalance || busy === 'allowance' || isPending}
          >
            {busy === 'allowance' ? <Loader2 size={14} className="animate-spin" /> : <RefreshCcw size={14} />}
            Save allowance
          </Button>
        </div>
      </section>
      ...
      <section className="rounded-xl border border-nativz-border bg-surface p-5">
        ...
        <p className="mt-1 text-[12px] text-text-muted">
          One-off grant or correction. Stripe top-ups land here automatically (Phase 5). Negative
          deltas are allowed for `adjust` (e.g. corrective claw-back).
        </p>
        <div className="mt-3 grid grid-cols-1 gap-3 sm:grid-cols-3">
          <label className="block">
            ...
            <select
              ...
              className="mt-1 w-full rounded-lg border border-nativz-border bg-background px-3 py-2 text-sm text-text-primary focus:border-accent focus:outline-none"
              disabled={!hasBalance || busy === 'grant'}
            >
        ...
        <div className="mt-4 flex justify-end">
          <Button
            ...
            size="sm"
            onClick={submitGrant}
            disabled={!hasBalance || busy === 'grant' || isPending || !grantDelta}
          >
            {busy === 'grant' ? (
      ...
      <section className="rounded-xl border border-nativz-border bg-surface p-5">
        ...
        <p className="mt-1 text-[12px] text-text-muted">
          Pausing skips the monthly allowance grant. Stripe top-ups and admin manual grants still
          work. Choose indefinite (until manually resumed) or time-bounded (auto-resume).
        </p>
        {paused ? (
          <div className="mt-3 rounded-lg border border-amber-300/30 bg-amber-300/5 p-3 text-[12px] text-amber-300">
            <p>
              ...
              {isPausedIndefinite
                ...
                : `Until ${
                    balance?.paused_until
                      ? new Date(balance.paused_until).toLocaleString('en-US')
                      : '—'
                  }.`}
            </p>
            {balance?.pause_reason ? <p className="mt-1">Reason: {balance.pause_reason}</p> : null}
            <div className="mt-3">
              <Button
                variant="secondary"
                size="sm"
      ...
      <section className="rounded-xl border border-nativz-border bg-surface p-5">
        ...
        <p className="mt-1 text-[12px] text-text-muted">
          Last {sortedTx.length} of the most recent transactions. Append-only audit log.
        </p>
        {sortedTx.length === 0 ? (
