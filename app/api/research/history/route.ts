...
export async function GET(request: NextRequest) {
  try {
    ...
    // viewers + admins impersonating) is strictly scoped to their
    // effective clientIds. Important: we pass allowedClientIds (exact
    // brand list) rather than organizationId, because orgs can host
    // multiple brands — scoping by org would leak sibling brands.
    const items = await fetchHistory({
      ...
      cursor,
      includeIdeas,
      allowedClientIds: ctx.role === 'viewer' ? ctx.clientIds ?? [] : null,
    });
