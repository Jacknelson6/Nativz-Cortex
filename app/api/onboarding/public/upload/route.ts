...
// Cap single-file uploads. Bigger files should use signed URLs; for a
// v1 that covers brand guides, logos, short clips, 50 MB is enough.
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (file.size > MAX_BYTES) {
      return NextResponse.json({ error: 'File is too large (50 MB max)' }, { status: 413 });
    }
    ...
    // Audit event + notification — non-blocking; never fails the client UI.
    ...
    void admin.from('onboarding_events').insert({
      tracker_id: tracker.id,
      kind: 'file_uploaded',
      phase_id: phaseId,
