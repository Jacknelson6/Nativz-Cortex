...
const createEdgeSchema = z.object({
  ...
  source_handle: z.string().max(50).optional().nullable(),
  target_handle: z.string().max(50).optional().nullable(),
...
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('GET /api/analysis/edges error:', error);
  ...
  try {
    ...
    const { data: edge, error: insertError } = await adminClient
      ...
      .insert({
        ...
        source_handle: parsed.data.source_handle || null,
        target_handle: parsed.data.target_handle || null,
    ...
    console.error('POST /api/analysis/edges error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
