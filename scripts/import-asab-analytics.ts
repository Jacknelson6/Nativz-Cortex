/**
 * Import All Shutters and Blinds (ASAB) analytics.
 * Facebook + Instagram from Meta Business Suite CSVs.
 *
 * Usage: npx tsx scripts/import-asab-analytics.ts
 */
...
const CLIENT_ID = 'e1b61d86-8c55-4c5b-b19c-a1542b41492d';
...
// Facebook: Views.csv + Follows.csv
...
// Instagram: Views (1).csv + Follows (1).csv
...
async function main() {
  ...
  const fbProfileId = await ensureProfile('facebook', 'All Shutters and Blinds');
  const igProfileId = await ensureProfile('instagram', 'allshuttersandblinds');
