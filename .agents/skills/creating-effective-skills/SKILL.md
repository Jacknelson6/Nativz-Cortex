...
description: Use when creating, designing, or improving Codex skills — covers skill taxonomy, architecture patterns, progressive disclosure, description optimization, gotchas sections, distribution strategies, and when to use scripts/data/hooks within skills. Also use when deciding what type of skill to build, how to structure skill folders, or how to distribute skills across a team.
---

# Creating Effective Skills

Based on lessons from Anthropic's internal use of hundreds of skills in Codex.
...
Codex already knows a lot about coding. Focus on information that pushes Codex **out of its normal way of thinking** — your org's specific patterns, edge cases, and opinions that differ from defaults.

### 2. Build a Gotchas Section

The highest-signal content in any skill. Build this up over time from actual failure points Codex hits. Update the skill as new gotchas emerge.
...
Use the folder structure as layered context. Tell Codex what files exist and when to read them.
...
Be specific about **what** Codex should know, but flexible about **how** it applies the knowledge. Overly rigid instructions break when the situation doesn't match your assumptions.
...
Codex scans skill descriptions to decide which to invoke. The description is a **triggering mechanism**, not a summary.
...
**Make descriptions slightly pushy** — Codex tends to under-trigger skills. Include adjacent phrases and contexts where the skill would help.
...
- **Append-only log**: `standups.log` — Codex reads history to know what changed
...
Give Codex to compose rather than reconstruct. If test runs show all subagents independently writing similar helper scripts, bundle that script in `scripts/`.
...
Check skills into `.Codex/skills/` in your repo. Every checked-in skill adds to model context, so be deliberate.
...
On first use, Codex asks for your preferences (preferred skill categories, distribution method, default location). Stored so future sessions don't re-ask.

### 2. `creation-log.md` — Skill Creation History
After each skill creation session, Codex appends an entry: what was built, what category, what worked, what didn't. Future sessions read this to spot patterns in your skill-building and avoid repeating mistakes.

### 3. `references/gotchas.md` — Accumulated Failures
When a skill doesn't trigger correctly, produces poor results, or hits an unexpected edge case, Codex adds the failure to this file with symptom, root cause, and fix. Read this before every new skill creation.
...
- [ ] Doesn't state things Codex already knows
