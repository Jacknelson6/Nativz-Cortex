...
description: >
  ...
  For React Bits source, NEVER read full ~/.Codex/skills/bedrock/source/*.txt files — grep for component delimiters then Read with offset+limit (max ~300 lines), or use reactbits.dev.
---
...
> `git clone https://github.com/Jacknelson6/bedrock.git ~/.Codex/skills/bedrock`
>
> **This folder** (`.Codex/skills/bedrock/`) carries **Shadcn blocks + single-prompt playbook** for Nativz Cortex; it does **not** duplicate the large `source/*.txt` files.
>
> **Same skill, any repo:** **[github.com/Jacknelson6/northstone-app-ui](https://github.com/Jacknelson6/northstone-app-ui)** — clone to `~/.Codex/skills/northstone-app-ui` for projects outside Cortex; keep this vendored copy in sync when you change the standalone repo.
...
| **Nativz product UI tokens & copy** | `.Codex/skills/frontend-patterns/SKILL.md` + `docs/detail-design-patterns.md` |
| **Style profiles, typography, motion theory (global Bedrock)** | [Jacknelson6/bedrock](https://github.com/Jacknelson6/bedrock) → `references/style-profiles.md` or `~/.Codex/skills/bedrock/references/` after clone |

## Cursor

- **`.cursor/rules/northstone-app-ui.mdc`** — applies when editing `app/**` and `components/**`; restates pipeline + delight budget.
- **`.Codex/commands/northstone-app-ui.md`** — slash **`/northstone-app-ui`** (distinct from global **`/bedrock`**).
...
5. **Context discipline**: React Bits source in **`~/.Codex/skills/bedrock/source/`** when [Jacknelson6/bedrock](https://github.com/Jacknelson6/bedrock) is installed — **grep** for `=== FILE:` then **Read** a slice; otherwise [reactbits.dev](https://reactbits.dev).
...
| Need | Where |
|------|--------|
| React Bits component code | `~/.Codex/skills/bedrock/source/reactbits-components.txt` |
| React Bits docs / props | `~/.Codex/skills/bedrock/source/reactbits-docs.txt` |
| Style profiles, tokens, templates | `~/.Codex/skills/bedrock/references/*.md` |

If those paths are missing, install **[Jacknelson6/bedrock](https://github.com/Jacknelson6/bedrock)** into `~/.Codex/skills/bedrock/` (see README there), **or** implement from [reactbits.dev](https://reactbits.dev) — do not invent APIs from memory.
...
- **Inside Nativz Cortex:** **`.Codex/skills/bedrock/`** (skill id **`northstone-app-ui`**) + **`.Codex/commands/northstone-app-ui.md`** (`/northstone-app-ui`) + **`.cursor/rules/northstone-app-ui.mdc`**.
...
- **React Bits source:** clone **[github.com/Jacknelson6/bedrock](https://github.com/Jacknelson6/bedrock)** to `~/.Codex/skills/bedrock` (per upstream README).
