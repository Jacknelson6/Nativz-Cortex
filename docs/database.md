...
## Service capacity → accounting (migrations 233-237)
...
- **237** extends `payroll_entries.source` with `'auto-deleted'` (tombstone) and folds it into the partial unique index. The DELETE handler at `app/api/accounting/entries/[id]/route.ts` flips auto/auto-edited rows to `auto-deleted` instead of hard deleting; the auto-populate engine treats the tombstone as a held slot so re-sync does not resurrect the row until the period closes. Every read site (period grid, totals, payouts, export, editor view) filters `source != 'auto-deleted'`.
