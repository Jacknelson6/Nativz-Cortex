# PRD: Content Lab — unified strategy brain

**Status:** Draft (living document)  
**Owner:** Product / eng  
**Last updated:** 2026-03-27  
**Related:** Content Lab UI (`/admin/content-lab/[clientId]`), The Nerd (`/admin/nerd`), analytics, knowledge vault, content pillars
...
**Content Lab** is the workspace where the team **builds** the funnel (topic search → pillars → brand DNA → named idea batches → analysis boards). **The Nerd** (and/or a dedicated strategy chat mode) is the **dialogue layer**: ask questions, compare posts, pressure-test pillars, and turn insights into shoot-ready direction.
...
- Replacing the full Brand DNA editor inside Content Lab (inline editing already exists; full page remains).
- Fully automated “video understanding” at scale without human-triggered analysis (phased).

## 4. Success metrics

- Strategists **prefer** Content Lab + Nerd for pillar reviews vs. ad-hoc docs.
...
- [x] PRD + backlog document (`docs/prd-content-lab-brain.md`)
- [x] `buildStrategyLabContextPack(clientId)` — live snapshot: content pillars, recent completed topic searches, brand DNA tone snippet (`lib/nerd/content-lab-context-pack.ts`)
...
### Phase B — Content Lab ↔ Nerd UX

- [x] Deep link `?strategyClient=` prefills prompt + mention (`app/admin/nerd/page.tsx`)
- [x] Strategy assistant card: link opens Nerd in a **new tab** to preserve the Content Lab workspace
- [x] `?strategySource=content-lab` sets a **session hint** so Nerd knows the conversation began from Content Lab
...
- [x] Content Lab: “Send to Cortex” action on analysis boards

### Phase F — Hardening — **implemented**

- [x] Rate limits on pack size; truncate pillar descriptions and cap total context pack size
- [x] E2E smoke: admin crawl now visits Content Lab, analytics client preselect, and Nerd Content Lab deep link
...
| Area | Path |
|------|------|
| Content Lab page | `app/admin/content-lab/[clientId]/page.tsx` |
| Workspace UI | `components/content-lab/content-lab-workspace.tsx`, `content-lab-content-stack-card.tsx` |
| Nerd chat API | `app/api/nerd/chat/route.ts` |
| Context pack builder | `lib/nerd/content-lab-context-pack.ts` |
| PRD / backlog | `docs/prd-content-lab-brain.md` |
...
The model should call these when the user asks for performance; the Content Lab **pack** is a static snapshot and does not replace live tool calls.
...
| 2026-03-27 | Added performance + affiliate lines to Content Lab pack, plus total pack truncation and unit test |
| 2026-03-27 | Added board/video strategy tools, Content Lab → Cortex board handoff, session hinting, and admin crawl smoke coverage |
