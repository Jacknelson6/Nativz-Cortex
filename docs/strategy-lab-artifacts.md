...
2. Click **Open in Strategy Lab** from the search results page.
...
- **Attached to a client:** writes `[search.id]` to the client's
  ...
  `router.push('/admin/strategy-lab/[clientId]')`. Single-pin replace —
  the user lands on exactly this search.
- **No client:** opens `StrategyLabAttachClientDialog`
  (`components/strategy-lab/strategy-lab-attach-client-dialog.tsx`),
...
to `/admin/strategy-lab/[clientId]`. The workspace's multi-pin state
(see below) loads the full array.

### Direct
`/admin/strategy-lab/[clientId]` from the sidebar. The workspace
auto-selects the most recent completed topic search if nothing is
pinned in localStorage.

## State plumbing

`strategy-lab-workspace.tsx` holds the canonical `pinnedTopicSearchIds: string[]`
state. On mount it:
...
The chip bar (`strategy-lab-topic-search-chip-bar.tsx`) receives the
...
The chat (`strategy-lab-nerd-chat.tsx`) receives `pinnedTopicSearchIds`
...
### Full conversation PDF — `strategy-lab-conversation-pdf.tsx`
...
`components/strategy-lab/pdf-markdown.tsx` to parse each assistant
message's markdown into react-pdf primitives.

**Mermaid handling** (April 11): before the PDF renders,
`components/strategy-lab/strategy-lab-conversation-export-button.tsx`
calls `rasterizeMermaidBlocks` (`lib/strategy-lab/rasterize-mermaid.ts`)
which:
...
`lib/nerd/strategy-lab-scripting-context.ts` exports
`STRATEGY_LAB_ADDENDUM` — a ~6400-char system prompt suffix that gets
appended whenever `/api/nerd/chat` receives `mode: 'strategy-lab'`. It
teaches the model:
...
A regression guard (`scripts/smoke-strategy-lab-addendum.ts`) asserts
...
`strategy-lab-nerd-chat.tsx#SUGGESTIONS` defines the quick-start pills
...
| `scripts/smoke-strategy-lab-addendum.ts` | 15 assertions on `STRATEGY_LAB_ADDENDUM` — all load-bearing sections + keywords + length budget. |
